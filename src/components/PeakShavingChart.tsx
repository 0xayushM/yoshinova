...
const PeakShavingChart: React.FC<PeakShavingChartProps> = ({
  ...
  const maxValue = Math.max(...data.map((d) => Math.max(d.withoutBESS, d.withBESS)));
  const yMax = Math.ceil(maxValue * 1.2);

  const toX = (hour: number) => padL + (hour / 24) * plotW;
  const toY = (val: number) => padT + plotH - (val / yMax) * plotH;

  const beforePoints = data.map((d) => ({ x: toX(d.hour), y: toY(d.withoutBESS) }));
  const afterPoints = data.map((d) => ({ x: toX(d.hour), y: toY(d.withBESS) }));
  ...
  for (let i = 0; i < data.length - 1; i++) {
    const diff = data[i].withoutBESS - data[i].withBESS;
      ...
      <svg viewBox={`0 0 ${chartW} ${chartH}`} className="w-full h-full" preserveAspectRatio="xMidYMid meet">
        ...
        <g ref={chargingAreasRef} opacity="0">
          {areas.filter(a => a.type === 'charging').map((area, i) => {
            ...
            const topPath = points.map(d => `${toX(d.hour)},${toY(d.withoutBESS)}`).join(' ');
            const bottomPath = points.slice().reverse().map(d => `${toX(d.hour)},${toY(d.withBESS)}`).join(' ');
            ...
            return (
              <g key={`charging-${i}`}>
                ...
                <text 
                  x={(startX + endX) / 2} 
                  y={toY(Math.max(...points.map(p => p.withoutBESS))) - 10}
                  textAnchor="middle"
                  fontSize="11"
                  fill="#1976D2"
                  fontWeight="600"
                >
                  BESS charging from generator
                </text>
              </g>
            );
          })}
          ...
          {areas.filter(a => a.type === 'discharging').map((area, i) => {
            ...
            return (
              <g key={`discharging-${i}`}>
                ...
                <text 
                  x={(startX + endX) / 2} 
                  y={toY(Math.min(...points.map(p => p.withBESS))) + 20}
                  textAnchor="middle"
                  fontSize="11"
                  fill="#D32F2F"
                  fontWeight="600"
                >
                  BESS discharge into network
                </text>
              </g>
            );
          })}

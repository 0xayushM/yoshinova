...
interface BESSComparisonChartProps {
  data: ChartDataPoint[];
  yAxisLabel?: string;
  yAxisUnit?: string;
  maxY?: number;
...
const BESSComparisonChart: React.FC<BESSComparisonChartProps> = ({
  data,
  ...
  const computedMax = maxY ?? Math.ceil(Math.max(...data.map((d) => Math.max(d.withoutBESS, d.withBESS))) * 1.15);
  const yTicks = 5;
  ...
  const withoutPoints = data.map((d) => ({ x: toX(d.hour), y: toY(d.withoutBESS) }));
  const withPoints = data.map((d) => ({ x: toX(d.hour), y: toY(d.withBESS) }));
  ...
  const savingsFillPath =
    ...
    data.slice().reverse().map((d) => ` L ${toX(d.hour)} ${toY(d.withBESS)}`).join("") +
    " Z";
  ...
  // Peak point (highest withoutBESS)
  const peakIdx = data.reduce((mi, d, i) => (d.withoutBESS > data[mi].withoutBESS ? i : mi), 0);
  const peakX = toX(data[peakIdx].hour);
  const peakY = toY(data[peakIdx].withoutBESS);
...
export default BESSComparisonChart;
